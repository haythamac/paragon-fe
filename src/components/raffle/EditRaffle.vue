...
watch(
    ...
    async (newVal) => {
        ...
        try {
            ...
            const showResponse = await raffleAPI.getById(props.raffleId);
            raffleData.value = showResponse.data.data;

            selectedMembers.value = raffleData.value.members
            selectedItems.value = raffleData.value.items.map(item => ({
                ...item,
                quantity: item.quantity ?? item.pivot?.initial_quantity ?? 1
            }))

            
            raffleName.value = raffleData.value.name
            raffleDate.value = raffleData.value.date.slice(0,10)
...
const handleSubmit = async () => {
    ...
    try {
        ...
        const response = await raffleAPI.update(props.raffleId, raffleData)
            ...
            <transition name="fade">
                <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4"
                    ...
                    <div role="dialog" aria-modal="true"
                        ...
                        <p class="text-sm text-gray-400 mb-4">These details already exists in the database.</p>
