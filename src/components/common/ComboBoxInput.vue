<template>
  <div class="relative w-full" ref="comboboxRef">
    <!-- Input with search -->
    <div class="relative">
      <input v-model="searchQuery" @focus="isOpen = true" @input="isOpen = true" type="text" :placeholder="placeholder"
    ...
    <div v-if="isOpen && filteredItems.length > 0"
      ...
      <div v-for="item in filteredItems" :key="item[itemValue]" @click="toggleSelection(item)" :class="[
...
const props = defineProps({
  ...
  formatDisplay: {  // NEW - optional formatter function
    type: Function,
    default: null
  }
});
